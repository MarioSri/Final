...
export const DocumentProvider: React.FC<DocumentProviderProps> = ({ children }) => {
  ...
  const getDocumentTypeFromUpload = (documentTypes: string[]): 'Letter' | 'Circular' | 'Report' => {
  ...
  const getPriorityMapping = (priority: string): 'low' | 'medium' | 'high' | 'urgent' => {
    switch (priority) {
      case 'low': return 'low';
  ...
  const createInitialWorkflow = (userRole: string, userName: string) => {
    const steps = [
  ...
  const addDocument = (documentData: any, userRole: string, userName: string) => {
